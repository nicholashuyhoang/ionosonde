...
def check_lock(u, log=None, exit_if_not_locked=False):
    ...
    locked=True #hack for external GPS
...
#        min_sync_time=20.0
    ...
    time_at_last_pps = u.get_time_last_pps().get_real_secs()
    while time_at_last_pps == u.get_time_last_pps().get_real_secs():
        time.sleep(0.1)
    u.set_time_next_pps(uhd.libpyuhd.types.time_spec(int(n.ceil(time.time()))));
...
def sync_clock_old(u, log, min_sync_time=300.0):
    # Based on the specs for the gpsdo, it takes 5 minutes to warm
    # up after restart
    t0=time.time()
    print(t0)
...
#        min_sync_time=20.0
    ...
    #u.set_clock_source("gpsdo") #<---ORIGINAL
    #u.set_time_source("gpsdo") #<---ORIGINAL
    u.set_clock_source("external")
    u.set_time_source("external")

    lastt=u.get_time_last_pps()
    nextt=u.get_time_last_pps()
    print(lastt.get_real_secs(), nextt.get_real_secs())
...
def sync_clock_original(u, log, min_sync_time=300.0):
    # Based on the specs for the gpsdo, it takes 5 minutes to warm
    # up after restart
    t0=time.time()
