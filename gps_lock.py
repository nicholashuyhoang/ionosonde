...
def check_lock(u, log=None, exit_if_not_locked=False):
    ...
    #commented out anything referrencing ^get_mboard_sensor so now this function will always return true
    locked = true
...
## this is the original function which is for reference while tinkering but actual used function is below this implementation

def original_sync_clock(u, log, min_sync_time=300.0):
...
#        min_sync_time=20.0
    ...
    # originally "gpsdo" but we will be using an external clock
...
# commented out anything that requires mother board calls
...
#        min_sync_time=20.0
    ...
    # original:"external" = "gpsdo" but we will be using an external clock
    ...
    # original: u.set_time_next_pps(uhd.libpyuhd.types.time_spec(u.get_mboard_sensor("gps_time").to_int()+1))
    # original: log.log(str(u.get_mboard_sensor("gps_gpgga")))
    # original: log.log(str(u.get_mboard_sensor("gps_gprmc")))
    
    # debugging purposes
    t_next = time.time() + 1
    print("t0 =", t0, "t_next = ", t_next," ", int(t_next))

    # this instead uses host computer clock
    ...
    # originial: t_gpsdo=u.get_mboard_sensor("gps_time")
    # these should be similar
    # originial: print("pc clock %1.2f usrp clock %1.2f gpsdo %1.2f" % (t_now, t_usrp, t_gpsdo.to_int()))
    print("pc clock %1.2f usrp clock %1.2f gpsdo %1.2f" % (t_now, t_usrp))    
